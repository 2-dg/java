...
public class EditGameController implements Initializable {
	...
	private GameVO game = ViewController.getSelectedGame();
	...
	public void handleBtnEdit() {
		...
		if(txtEGhomeName.getText().isEmpty()||txtEGawayName.getText().isEmpty()) {
			...
			alert.showAndWait();
				...
				gameEdit = new GameVO(dpEGgamedate.getValue().toString(),
         ...
         stage.close();
         gameDAO = new GameDAO();
         gameDAO.gameEdit(gameEdit, game.getGameNo());
         ViewController.listOfGames.removeAll(ViewController.listOfGames);
         ViewController.totalListGM();
		}
	}
