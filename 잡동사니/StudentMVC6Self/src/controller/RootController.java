...
//화면 ID설정=====================================================
   @FXML TableView<StudentVO> tableView=new TableView<>();   //테이블뷰 사용
   ...
   @FXML Button btnSum;          //총점
   @FXML Button btnImageFile;
...
//   ObservableList<Student> selectStudent;
   ObservableList<StudentVO> data=FXCollections.observableArrayList();  //테이블뷰에 연동될 ArrayList
   ObservableList<StudentVO> selectStudent=null;
...
//      테이블 컬럼생성 및 해당된 컬럼에 사이즈와 스타일(가운데정렬), 해당클래스와 매칭시킨다.
      ...
      tableView.getColumns().addAll(colNo, colName, colLevel, colBan, colGender, colKorean, 
            colEnglish, colMath, colSic, colSoc, colMusic, colTotal, colAvg, colRegister, colFileName);
//      totalList();
...
//            System.out.println("행 번호 : "+selectedIndex);
            ...
            cbYear.setValue(selectStudent.get(0).getYear());           
         }
      });
...
//            }
            ...
            data.removeAll(data);
            StudentVO svo = null;
            StudentDA0 sdao = new StudentDA0();
